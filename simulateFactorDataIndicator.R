...
sampleData <- function() {
  ...
  data <- parLapply(cl, seq_len(setParam$dgp$nSamples), function(iSample) {
    ...
    if (factors) {
      ...
      popModelIndicators <- genModel(colnames(X_full_wME), setParam$dgp$interDepth, setParam$dgp$poly)
      
      # predictor matrix that allows for polynomials and interactions
      X_full_final <- model.matrix(as.formula(popModelIndicators), data.frame(X_full_wME))
      
      # remove first degree polynomials from data (they are duplicates!)
      X_full_final <- rmDuplicatePoly(X_full_final)
    ...
    list(yMat = yMatrix,
         # this is the data if factors = {TRUE, FALSE}; identical dimensions with factors or single indicators
         X_int = X_final, # save predictors with measurement error (with sum score for factors)
         # this only exists if factors = TRUE; but NA matrix if factors = FALSE
         # -> all indicators as predictors (number of factors * number of indicators per factor) 
         X_full_wME = if(factors) X_full_wME else matrix(NA, ncol = (setParam$dgp$p * setParam$dgp$nIndicator) + pTrash, nrow = N),
         # -> 
         X_full_final = if (factors) X_full_final else matrix(
           NA, ncol = ((setParam$dgp$p * setParam$dgp$nIndicator) + pTrash) * 
             setParam$dgp$poly + choose(setParam$dgp$p * setParam$dgp$nIndicator + pTrash, setParam$dgp$interDepth)),
         # data without measurement error
         # X_int = X_int, # with factors or without multiple indicators per factor
