...
# resFolder <- "results/resultsFactorsWithoutInteraction"
resFolder <- "results/resultsFactorsWithInteraction"
...
plotRelBias <- function(data) { # plot function
  ggplot(data,
         aes(x = interaction(pTrash, N, sep = " x "), y = relBiasB, 
             group = interaction(R2, factor), colour = R2,
...
selectedVars <- tidyr::unite(selectedVars, "N_pTrash_rel_f_R2_lin_inter", 
                             c(N, pTrash, rel, factor, R2, lin_inter),
                             sep = "_", remove = FALSE) 
subLists <- split(selectedVars, 
                  f = selectedVars$N_pTrash_rel_f_R2_lin_inter)
...
resSampleStats <- lapply(seq_along(subLists), function(iCond) {
  ...
  cbind(N_pTrash_rel_f_R2_lin_inter = names(subLists)[iCond], 
...
resSampleStats <- tidyr::separate(resSampleStats, N_pTrash_rel_f_R2_lin_inter, 
                                  into = c("N", "pTrash", "rel", "factor", "R2", "lin", "inter"), sep = "_")
resSampleStats <- tidyr::unite(resSampleStats, "lin_inter", c(lin, inter), sep = "_")
resSampleStats <- tidyr::pivot_longer(resSampleStats, !c(N, pTrash, rel, factor, R2, lin_inter), 
